...
There has been Alert from your driver please check on him"""


factory = PiGPIOFactory(host='')#your ip address
red = LED(17,pin_factory=factory)
bz = Buzzer(27,pin_factory=factory)
def sendmail():
    try:
        ...
        if rect!=None:
            ...
            if eye_aspect_ratio < EYE_DROWSINESS_THRESHOLD:
                ...
                if time.time()-eye_start_time >= EYE_DROWSINESS_INTERVAL:
                    ...
                    if time.time()-eye_start_time >= 7.00:
                     print("car is stopping")
                     cv2.putText(frame, "car is stopping...\nPLEASE TAKE A BREAK!", (10, 20),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)
                     red.on()
                     bz.on()
                     sendmail()
